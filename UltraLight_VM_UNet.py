...
class PVMLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        assert (
            C == self.input_dim
        ), f"Expected input channels: {self.input_dim}, but got: {C}"
...
class Channel_Att_Bridge(nn.Module):
    def __init__(self, c_list, split_att="fc"):
        ...
        self.att1 = (
            nn.Linear(c_list_sum, c_list[0])
            if split_att == "fc"
            else nn.Conv1d(c_list_sum, c_list[0], 1)
        )
        self.att2 = (
            nn.Linear(c_list_sum, c_list[1])
            if split_att == "fc"
            else nn.Conv1d(c_list_sum, c_list[1], 1)
        )
        self.att3 = (
            nn.Linear(c_list_sum, c_list[2])
            if split_att == "fc"
            else nn.Conv1d(c_list_sum, c_list[2], 1)
        )
        self.att4 = (
            nn.Linear(c_list_sum, c_list[3])
            if split_att == "fc"
...
class UltraLight_VM_UNet(nn.Module):
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            ...
            fan_out //= m.groups
            m.weight.data.normal_(0, torch.sqrt(torch.tensor(2.0 / fan_out)))
            if m.bias is not None:
                m.bias.data.zero_()
    ...
    def forward(self, x):
        ...
        # (@sna): When you use F.max_pool2d with a kernel size of 2 and a
        # stride of 2, the output dimensions are halved. However, when you
        # use F.interpolate, the resulting dimensions can sometimes be off by
        # one pixel, especially when the input dimensions are not perfectly
        # divisible by the pooling factors.
        # Instead of relying on the output of the previous layer, we
        # explicitly set the size for the interpolation to ensure it matches t3.
        out4 = F.gelu(
            F.interpolate(
                self.dbn2(self.decoder2(out5)),
                size=(t4.size(2), t4.size(3)),
                mode="bilinear",
                align_corners=True,
            )
        )  # b, c3, H/16, W/16
